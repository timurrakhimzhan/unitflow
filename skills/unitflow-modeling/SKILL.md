...
description: "Use when designing, implementing, reviewing, or testing Unitflow/effect-model UI state manager code: Effect Service models, Store/Event ports, Resource/Mutation async flows, React View.make bindings, dynamic keyed model instances, scoped lifetime, Registry.allSettled tests, or migrations away from atom-based UI state."
---
...
3. Return exactly three surfaces: `inputs`, `outputs`, `ui`.
...
- Use `Resource` for reads, `Mutation` for observable writes, and plain Effects
