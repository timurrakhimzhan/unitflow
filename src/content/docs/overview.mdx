...
Unitflow is an Effect runtime for React.

A Unitflow model is an Effect service that encapsulates a component's
business logic. It owns state, actions, async work, child models, and
lifetime. It can depend on other UI models and regular Effect services
through layers. A View binds the model to the screen at the edge, and — for
a model it leases itself — owns its lifecycle too: mounting leases it,
unmounting releases it.

```txt
Model     -> Effect service encapsulating a component's logic
Store     -> model state
Event     -> model action
Query     -> async read state
Mutation  -> async write state
View      -> render binding, owns lifecycle for models it leases itself
```

## Philosophy

Unitflow is an attempt at a middle point: flexible enough that logic of any
complexity is easy to build out of Effect and the primitives it gives you,
but not so flexible that every model ends up reinventing its own shape.
Enough freedom to build anything; not enough freedom to build it ten
different ways.
